...
const DisplayCustomer = ({ customer, services, onBookAppointment, onRecommendServices, onAddVehicle }) => {
  const [expanded, setExpanded] = useState(false);
  const [showBookForm, setShowBookForm] = useState(false);
  const [showRecommendForm, setShowRecommendForm] = useState(false);
  const [showVehicleForm, setShowVehicleForm] = useState(false);
  const [bookData, setBookData] = useState({ vehicle: '', service_type: '', scheduled_at: '' });
  const [recData, setRecData] = useState({ vehicle: '', services: [], note: '' });
  const [vehicleData, setVehicleData] = useState({ make: '', model: '', year: '', license_plate: '' });
    ...
    <div className={`customer-card ${expanded ? 'expanded' : ''}`}>
      <div className="customer-card-header" onClick={() => setExpanded(!expanded)}>
        <div className="customer-card-summary">
          <h5 style={{ color: '#2F6DAB', margin: 0 }}>{first_name} {last_name}</h5>
          <span className="customer-card-info">
            <a href={`mailto:${email}`} onClick={e => e.stopPropagation()}>{email}</a>
            {phone && <a href={`tel:${phone}`} onClick={e => e.stopPropagation()}>{phone}</a>}
          </span>
        </div>
          ...
          {vehicles.length > 0 && (
            <div className="dropdown">
              ...
              <div className="dropdown-content">
                {vehicles.map(v => (
                  <p key={v.vehicle_id}>{v.year} {v.make} {v.model}</p>
                ))}
              </div>
            </div>
          )}
              ...
              <div className="dropdown-content">
                {appointments.slice(0, 3).map(a => (
                  <p key={a.appointment_id}>
                    {a.scheduled_at ? new Date(a.scheduled_at).toLocaleDateString() : '—'} – {a.service_type}
                  </p>
                ))}
                {appointments.length > 3 && (
                  <p style={{ color: '#9ca3af', fontStyle: 'italic' }}>+{appointments.length - 3} more</p>
                )}
              </div>
            </div>
          )}
      ...
      {expanded && (
        <div className="customer-card-body">
          ...
          <div className="customer-section">
            ...
            {vehicles.length > 0 ? (
              <table className="customer-table">
                <thead>
                  <tr><th>Year</th><th>Make</th><th>Model</th><th>Plate</th></tr>
                </thead>
                <tbody>
                  {vehicles.map(v => (
                    <tr key={v.vehicle_id}>
                      <td>{v.year}</td><td>{v.make}</td><td>{v.model}</td><td>{v.license_plate}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ) : (
              <p style={{ color: '#9ca3af', margin: '4px 0' }}>No vehicles registered.</p>
            )}
            <button
              className="btn-action btn-add-vehicle"
              onClick={() => { setShowVehicleForm(!showVehicleForm); setShowBookForm(false); setShowRecommendForm(false); }}
            >
              + Add Vehicle
            </button>
            ...
            {showVehicleForm && (
              <form className="customer-inline-form" onSubmit={handleAddVehicle}>
                <input type="text" placeholder="Make (e.g. Toyota)" value={vehicleData.make} onChange={e => setVehicleData({ ...vehicleData, make: e.target.value })} required />
                <input type="text" placeholder="Model (e.g. Camry)" value={vehicleData.model} onChange={e => setVehicleData({ ...vehicleData, model: e.target.value })} required />
                <input type="number" placeholder="Year (e.g. 2023)" min="1900" max="2099" value={vehicleData.year} onChange={e => setVehicleData({ ...vehicleData, year: e.target.value })} required />
                <input type="text" placeholder="License Plate" value={vehicleData.license_plate} onChange={e => setVehicleData({ ...vehicleData, license_plate: e.target.value })} required />
                <div className="customer-form-actions">
                  <button type="submit" className="btn-action btn-add-vehicle">Save Vehicle</button>
                  <button type="button" className="btn-action btn-cancel" onClick={() => setShowVehicleForm(false)}>Cancel</button>
                </div>
              </form>
            )}
          </div>
          ...
          <div className="customer-action-buttons">
            <button className="btn-action btn-book" onClick={() => { setShowBookForm(!showBookForm); setShowRecommendForm(false); }}>
              Book Appointment
            </button>
            <button className="btn-action btn-recommend" onClick={() => { setShowRecommendForm(!showRecommendForm); setShowBookForm(false); }}>
              Recommend Services
            </button>
          </div>
          ...
          {showBookForm && (
            <form className="customer-inline-form" onSubmit={handleBook}>
              <select value={bookData.vehicle} onChange={e => setBookData({ ...bookData, vehicle: e.target.value })} required>
              ...
              <select value={bookData.service_type} onChange={e => setBookData({ ...bookData, service_type: e.target.value })} required>
              ...
              <input
                ...
                onChange={e => setBookData({ ...bookData, scheduled_at: e.target.value })}
                required
              />
              <div className="customer-form-actions">
                <button type="submit" className="btn-action btn-book">Confirm Booking</button>
                <button type="button" className="btn-action btn-cancel" onClick={() => setShowBookForm(false)}>Cancel</button>
              </div>
            </form>
          )}

          {/* Recommend Services Form */}
          {showRecommendForm && (
            <form className="customer-inline-form" onSubmit={handleRecommend}>
              <select value={recData.vehicle} onChange={e => setRecData({ ...recData, vehicle: e.target.value })} required>
                <option value="">Select Vehicle</option>
                {vehicles.map(v => (
                  <option key={v.vehicle_id} value={v.vehicle_id}>
                    {v.year} {v.make} {v.model} ({v.license_plate})
              ...
              <div className="service-checklist">
                <label style={{ fontWeight: 600, marginBottom: 4 }}>Select Services:</label>
                {services.map(s => (
              ...
              <textarea
                ...
                value={recData.note}
                onChange={e => setRecData({ ...recData, note: e.target.value })}
                rows={2}
              />
              <div className="customer-form-actions">
                <button type="submit" className="btn-action btn-recommend" disabled={recData.services.length === 0}>
                  Send Recommendations
                </button>
                <button type="button" className="btn-action btn-cancel" onClick={() => setShowRecommendForm(false)}>Cancel</button>
              </div>
            </form>
          )}
          ...
          <div className="customer-section">
            ...
            {appointments.length === 0 ? (
              ...
              <div className={`service-history-list ${appointments.length > 10 ? 'scrollable' : ''}`}>
                <table className="customer-table">
                  <thead>
                    <tr><th>Date</th><th>Service</th><th>Vehicle</th><th>Cost</th><th>Status</th></tr>
                  </thead>
                  <tbody>
                    {appointments.map(a => (
...
const CustomerList = () => {
  ...
  const handleBookAppointment = async (customerId, bookData) => {
    ...
    if (token) headers["Authorization"] = `Bearer ${token}`;
    try {
      const res = await fetch(`${API_BASE_URL}/api/admin/customers/${customerId}/book-appointment/`, {
        method: 'POST', headers, body: JSON.stringify(bookData),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        return { ok: false, error: data.detail || 'Failed to book appointment.' };
      }
      const created = await res.json();
      // Add the new appointment to the customer's local list
  ...
  const handleRecommendServices = async (customerId, recData) => {
    ...
    if (token) headers["Authorization"] = `Bearer ${token}`;
    try {
      const res = await fetch(`${API_BASE_URL}/api/admin/customers/${customerId}/recommend-services/`, {
        method: 'POST', headers, body: JSON.stringify(recData),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        return { ok: false, error: data.detail || 'Failed to send recommendations.' };
      }
      return { ok: true };
    } catch (err) {
      return { ok: false, error: err.message };
  ...
  const handleAddVehicle = async (customerId, vehicleData) => {
    ...
    if (token) headers["Authorization"] = `Bearer ${token}`;
    try {
      const res = await fetch(`${API_BASE_URL}/api/admin/customers/${customerId}/vehicles/`, {
        method: 'POST', headers, body: JSON.stringify(vehicleData),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        return { ok: false, error: data.detail || 'Failed to add vehicle.' };
      }
      const created = await res.json();
      setCustomers(prev => prev.map(c =>
        c.customer_id === customerId
  ...
  return (
    <section className="admin-dashboard">
      ...
      <div className="admin-dashboard-content ms-md-5">
        ...
        <AdminNewCustomer isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} />
      </div>
    </section>
  )
}
